    ...
    return DashboardData(
      ...
      copyTrades:
          (json['copyTrades'] as List<dynamic>?)
              ?.map((t) => CopyTrade.fromJson(Map<String, dynamic>.from(t as Map)))
              .toList() ??
          [],
      pendingDeposits:
          (json['pendingDeposits'] as List<dynamic>?)
              ?.map((d) => PendingDeposit.fromJson(Map<String, dynamic>.from(d as Map)))
              .toList() ??
          [],
      ranks:
...
class CopyTrade {
  ...
  factory CopyTrade.fromJson(Map<String, dynamic> json) {
    return CopyTrade(
      ...
      closeAt: json['closeAt'] != null ? DateTime.tryParse(json['closeAt'].toString()) : null,
    );
  }
}
...
class DashboardProvider extends ChangeNotifier with WidgetsBindingObserver {
  ...
  void _startProfitProgression(CopyTrade trade, double targetProfit, void Function() onUpdate, void Function() onComplete) {
    _tradeTargetProfits[trade.id] = targetProfit;
    final totalTicks = _tradeDuration.inMilliseconds ~/ _profitTick.inMilliseconds;
    var tick = 0;
    final random = Random();
  ...
  void _updateTradeProfit(String tradeId, double profit) {
    ...
    if (updated != null && _data != null) {
      _data = _reconcileRanks(DashboardData(
  ...
  void _completeTrade(String tradeId) {
    ...
    final wb = profile.withdrawableBalance + finalProfit;
    profile = InvestmentLogic.withResolvedRank(
      profile.copyWith(withdrawableBalance: wb),
      seeds,
    );
    ...
    _data = _reconcileRanks(DashboardData(
      ...
      dailyRewardAmount: _data!.dailyRewardAmount,
      copyTradesLimit: profile.rank?.copyTradesLimit ?? _data!.copyTradesLimit,
      minDeposit: _data!.minDeposit,
      minWithdrawal: _data!.minWithdrawal,
    ));
  ...
  bool get _useLocalOnly => _apiService.baseUrl.contains('your-backend-api.com') || _apiService.baseUrl.contains('10.0.2.2');

  /// True when running on a local dev machine (127.0.0.1 or localhost).
  bool get _isDevEnvironment => _apiService.baseUrl.contains('127.0.0.1') || _apiService.baseUrl.contains('localhost');
  ...
  bool get canClaimRankDailyProfit {
    ...
    return InvestmentLogic.canClaimRankDailyProfitToday(synced) &&
        InvestmentLogic.rankForPrincipal(synced.totalBalance, _seedRanks) != null &&
  ...
  DashboardData _buildLocalDashboard(String? userId) {
    ...
    final ranks = InvestmentLogic.ranksWithCurrentFlag(_seedRanks, profile.rank?.id);
  ...
  Future<void> fetchDashboardData({String? userId, bool showLoading = true}) async {
      ...
      if (_useLocalOnly) {
        ...
        if (res.success && res.data != null) {
          ...
          if (_data?.profile != null) {
            final synced = InvestmentLogic.withResolvedRank(_data!.profile!, _seedRanks);
            _data = _replaceProfile(_data!, synced);
            _data = _reconcileRanks(_data!);
          }
          await fetchFinance();
        } else {
          _errorMessage = res.message.isNotEmpty ? res.message : 'Dashboard unavailable';
          if (_data == null) {
            _data = _buildLocalDashboard(_userId ?? userId);
            _data = _reconcileRanks(_data!);
          }
    ...
    try {
      ...
      if (res.success && res.data != null) {
        ...
        if (_financeData?.profile != null && _data != null) {
          final merged = InvestmentLogic.withResolvedRank(_financeData!.profile!, _seedRanks);
          _data = _reconcileRanks(
            DashboardData(
              ...
              canClaimDaily: _data!.canClaimDaily,
              dailyRewardAmount: _data!.dailyRewardAmount,
              copyTradesLimit: merged.rank?.copyTradesLimit ?? _data!.copyTradesLimit,
      ...
      for (final w in _financeData!.withdrawals) {
        if (w.status == 'delivered' && _notifiedWithdrawalIds.add(w.id)) {
  ...
  void _rebuildLocalFinanceSnapshot() {
    final overview = FinanceOverview(
      totalDeposits: _localDeposits.where((d) => d.status == 'approved').fold<double>(0, (s, d) => s + d.amount),
      pendingDeposits: _localDeposits.where((d) => d.status == 'pending').fold<double>(0, (s, d) => s + d.amount),
      totalWithdrawals: _localWithdrawals.where((w) => w.status == 'approved').fold<double>(0, (s, w) => s + w.amount),
  ...
  void _expireApprovedDeposits() {
    ...
    for (var i = 0; i < _localDeposits.length; i++) {
      ...
      if (d.status == 'approved' && d.expiresAt != null && d.expiresAt!.isBefore(now)) {
        _localDeposits[i] = d.copyWith(status: 'expired');
        totalExpired += d.amount;
      }
    }
    if (totalExpired > 0 && _data?.profile != null) {
      final newLocked = (_data!.profile!.lockedBalance - totalExpired).clamp(0.0, double.infinity);
  ...
  Future<UserDeposit?> submitDeposit({
    ...
    try {
      ...
      if (_useLocalOnly) {
        ...
        final exp = DateTime.now().add(Duration(days: InvestmentLogic.lockDays));
  ...
  Future<UserWithdrawal?> submitWithdrawal({
    ...
    try {
      ...
      final token = await AuthService.getToken();
      final api = ApiService(baseUrl: _apiService.baseUrl, authToken: token);
      final res = await api.postUserWithdrawal(
        amount: amount,
        network: network,
        walletAddress: walletAddress.trim(),
      );
  ...
  Future<bool> devApproveDeposit(String id) async {
    ...
    if (_useLocalOnly) {
      final i = _localDeposits.indexWhere((d) => d.id == id && d.status == 'pending');
      if (i < 0) return false;
    ...
    if (_isDevEnvironment) {
      try {
        await http.post(
          Uri.parse('${_apiService.baseUrl}/api/admin/deposits/$id/approve'),
  ...
  Future<bool> devApproveWithdrawal(String id) async {
    if (_useLocalOnly) {
      final i = _localWithdrawals.indexWhere((w) => w.id == id && w.status == 'pending');
      if (i < 0) return false;
    ...
    if (_isDevEnvironment) {
      try {
        await http.post(
          Uri.parse('${_apiService.baseUrl}/api/admin/withdrawals/$id/approve'),
          headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer admin_demo_token'},
        ).timeout(const Duration(seconds: 10));
        await fetchDashboardData(userId: _userId, showLoading: false);
        await fetchFinance();
  ...
  Future<bool> devRejectWithdrawal(String id) async {
    ...
    if (_useLocalOnly) {
      ...
      _localWithdrawals[i] = w.copyWith(status: 'rejected', processedAt: DateTime.now());
      final refund = _data!.profile!.withdrawableBalance + w.amount;
      _updateDataWithProfile(_data!.profile!.copyWith(withdrawableBalance: refund));
      _rebuildLocalFinanceSnapshot();
      notifyListeners();
      return true;
    }
    ...
    if (_isDevEnvironment) {
      try {
        await http.post(
          Uri.parse('${_apiService.baseUrl}/api/admin/withdrawals/$id/reject'),
          headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer admin_demo_token'},
        ).timeout(const Duration(seconds: 10));
        await fetchDashboardData(userId: _userId, showLoading: false);
        await fetchFinance();
  ...
  DashboardData _reconcileRanks(DashboardData d) {
    final p = d.profile;
    if (p == null) return d;
    final resolved = InvestmentLogic.rankForPrincipal(p.totalBalance, _seedRanks);
    ...
    final limit = resolved?.copyTradesLimit ?? p.rank?.copyTradesLimit ?? d.copyTradesLimit;
  ...
  Future<bool> claimDailyReward() async {
    try {
      if (_useLocalOnly) {
        ...
        if (_data?.profile != null) {
          final updatedProfile = _data!.profile!.copyWith(
            withdrawableBalance: _data!.profile!.withdrawableBalance + DAILY_REWARD_AMOUNT,
            lastDailyRewardAt: DateTime.now(),
          );
          _updateDataWithProfile(InvestmentLogic.withResolvedRank(updatedProfile, _seedRanks));
        }
        notifyListeners();
        return true;
      }
      ...
      if (res.success && res.data != null && res.data!['profile'] != null) {
        final p = Profile.fromJson(Map<String, dynamic>.from(res.data!['profile'] as Map));
        final merged = p.userId.isEmpty && _userId != null ? p.copyWith(userId: _userId!) : p;
        _updateDataWithProfile(InvestmentLogic.withResolvedRank(merged, _seedRanks));
        notifyListeners();
        return true;
      }
      _errorMessage = res.message;
  ...
  Future<bool> claimRankDailyProfit() async {
    try {
      ...
      final rank = InvestmentLogic.rankForPrincipal(profile.totalBalance, seeds);
      if (rank == null || profile.lockedBalance <= 0) {
        _errorMessage = 'No eligible rank or zero locked balance';
        notifyListeners();
        return false;
      }
  ...
  Future<CopyTrade?> simulateCopyTrade() async {
    ...
    try {
      ...
      if (profile.lockedBalance <= 0) {
        _errorMessage = 'Deposit first — locked balance is required for copy trading.';
        _isSimulating = false;
        notifyListeners();
        return null;
      }
      ...
      if (_copyTradeSimTimestamps.length >= limit) {
        _errorMessage = 'Copy trade limit reached for your rank — $limit trade(s) per day (resets 12AM WAT).';
        _isSimulating = false;
        notifyListeners();
        return null;
      }
      ...
      final potentialDailyProfit = profile.lockedBalance * (dailyProfitPct / 100.0);
      final targetProfit = (potentialDailyProfit / limit).clamp(0.01, double.infinity);
  ...
  Future<Map<String, dynamic>> submitPaystackDeposit({
    ...
    if (res.success && res.data != null) {
      ...
      if (reference.isNotEmpty && authUrl.isNotEmpty) {
        ...
        await fetchDashboardData(userId: _userId, showLoading: false);
        await fetchFinance();
        return {'success': true, 'reference': reference, 'authorization_url': authUrl};
      }
    }
    _errorMessage = res.message;
    notifyListeners();
  ...
  double? potentialRankDailyProfit() {
    ...
    final rank = InvestmentLogic.rankForPrincipal(p.totalBalance, _seedRanks);
