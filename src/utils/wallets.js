...
export function getAssignment(depositId) {
  const a = activeAssignments.get(depositId);
  if (!a) return null;
  if (Date.now() > a.expires_at) {
    activeAssignments.delete(depositId);
    return null;
  }
  return a;
}

export function releaseAssignment(depositId) {
  activeAssignments.delete(depositId);
}
